...
typedef double (*PC_type)(unsigned n, const double *x, double *grad,
                          void *SS_ref_db);
...
typedef struct global_variables {
  ...
  int Mode; /** calcultion mode, 0 = full minimization, 1 = extract solution
               phases informations, 2 = local minimization */
  double **pdev;
  int n_em_db;
  char *research_group;
  int EM_database;
  ...
  int fluidSpec; /** activate fluid speciation along with phase equilibrium
                    modelling? */
  ...
  int max_n_mSS; /** maximum number of metastable solution phases pseudocompound
                    saved */
  int max_ss_size_cp;
  int len_cp;
  ...
  double
      *delta_pp_n; /** fraction of pure phase in estimated phase assemblage */
  ...
  double em2ss_shift; /** small value to retrieve x-eos from pure endmember
                         after levelling */

  /* PSEUDOCOMPOUNDS */
  double bnd_filter_pc;  /** value of driving force the pseudocompound is
                            considered to reduce the compositional space */
  double bnd_filter_pge; /** value of driving force the pseudocompound is
                            considered to reduce the compositional space */
  double max_G_pc;
  int *n_SS_PC;
  int *act_PP;
  ...
  double box_size_mode_PGE; /** edge size of the hyperdensity used during local
                               minimization */
  double box_size_mode_LP;  /** edge size of the hyperdensity used during local
                               minimization */
  int maxeval; /** maximum number of objective function evaluations during local
                  minimization */
  double bnd_val; /** boundary value for x-eos when the fraction of an endmember
                     = 0. */
  double obj_refine_fac; /** how much the residual of the objective function is
  ...
  double ur_1;    /** under relaxing factor on mass constraint if iteration is
                     bigger than it_1     */
  int it_2;       /** second critical iteration       */
  double ur_2;    /** under relaxing factor on mass constraint if iteration is
                     bigger than it_2     */
  int it_3;       /** third critical iteration       */
  double ur_3;    /** under relaxing factor on mass constraint if iteration is
                     bigger than it_3     */
  int it_f; /** send a failed message when the number of iteration is greater
               than this value */
  ...
  double G_system_mu; /** Gibbs energy of the system based on fraction and
                         chemical potential of phases **/

  double br_max_tol; /** max residual on mass-constraint */
  double alpha;      /** active under-relaxing factor of PGE, used to check if a
                        phase can be reintroduced */

  /* PHASE UPDATE */
  int *n_ss_array;
  int ph_change;
  double merge_value; /** norm distance between two instance of a solution phase
                         under which the instances are merged into 1 */
  double re_in_n;     /** fraction of phase when being reintroduce.  */
  double re_in_df;    /** driving force under which a phase can be added back to
                         the assemblage */
  double min_df;
  ...
  double gb_P_eps; /** small value to calculate V using finite difference: V =
                      dG/dP */
  double gb_T_eps; /** small value to calculate V using finite difference: V =
                      dG/dP */
...
/**
        definition of the objective function type in order to associate them
   with the right solution phase number
*/
typedef double (*obj_type)(unsigned n, const double *x, double *grad,
                           void *SS_ref_db);
...
typedef struct simplex_datas {
  ...
  double dG_B_tol; /** This is the minimum driving force under which the PC is
                      considered 		*/
  double min_F_tol;
  ...
  int swp;      /** swap occured?
                 */
  int *pivot;   /** pivot point when doing RREF toget the rational basis of the
                   null space 	*/
...
/* Declare structures to hold reference gbase, composition and factor for solid
 * solutions */
/* "bi","cpx","crd","ep","fl","g","amp","ilm","liq","mu","ol","opx","fsp","spl"
 */
typedef struct SS_refs {
  double P; /** used to pass to local minimizer, would allow to have pressure
               difference for liq/solid */
  double T;
  double R;
  int len_ox;
  ...
  double *G_pc;     /** array to store the gibbs energy of the pseudocompounds
                     */
  double *DF_pc;    /** array to store the final driving force of the
                       pseudocompounds 			*/
  double **comp_pc; /** compositional array of the pseudocompounds
                     */
  double **p_pc;    /** compositional array of the pseudocompounds
                     */
  // double **mu_pc;				/** compositional array of the
  // pseudocompounds */
  double **xeos_pc;  /** x-eos array of the pseudocompounds
                      */
  double *factor_pc; /** normalization factor of each PC, mainly useful for
                        liquid 				*/
  ...
  double *G_Ppc;     /** array to store the gibbs energy of the pseudocompounds
                      */
  double *DF_Ppc;    /** array to store the final driving force of the
                        pseudocompounds 			*/
  ...
  double **mu_array; /** 2d array of gbase, including values for numerical
                        differentiation 		*/
  double *gb_lvl;
  ...
  double *d_em;
  double *z_em; /** 1d array to deactivate endmembers when bulk-rock = 0; this
                   part is needed to calculat xi in PGE method */
  int n_guess;  /** number of initial guesses used to solve for solvi (or local
                   minimum) 	*/
  ...
  double *dfx;    /** gradient of objective function
                   */
  double **dp_dx; /** partial derivative of endmember fraction as function of
  ...
  double *xi_em;   /** endmember fraction calculated for PGE method (depends on
                      exp expression) */
...
typedef struct OUTDATA {
  ...
  double **xEOS; /* Compositional variables for each stable EM within a solid
                    solution	*/
  double **p_EM; /* Proportions of each of the endmembers   within a solid
                    solution		*/

} out_data;

/* structure to store position of zeros and non-zeros positions in bulk_rock
 * composition */
typedef struct bulk_infos {
  ...
  double *bulk_rock; /** bulk rock composition in weight
                      */
  double
      *bulk_rock_cat; /** bulk rock composition in weight (nzer values first) */
...
/* structure to informations about the considered set of phases during
 * minimization 	*/
...
typedef struct stb_systems {
  ...
  double *bulk_S;
  double frac_S;
  double rho_S; /* Solid system informations
                 */
  double *bulk_M;
  double frac_M;
  double rho_M; /* Melt system informations
                 */
  double *bulk_F;
  double frac_F;
  double rho_F; /* Fluid system informations
                 */

  double *bulk_S_wt;
  double frac_S_wt;
  double frac_S_vol; /* Solid system informations
                      */
  double *bulk_M_wt;
  double frac_M_wt;
  double frac_M_vol; /* Melt system informations
                      */
  double *bulk_F_wt;
  double frac_F_wt;
  double frac_F_vol; /* Fluid system informations
                      */
  ...
  int *ph_type;        /* 0 -> Solution phases; 1 -> Pure phases
                        */
  int *ph_id; /* position in the related stb_SS_phase or stb_PP_phase structure
                 arrays	*/
...
global_variable global_variable_init(global_variable gv, bulk_info *z_b);
...
typedef struct outdataCPP {
  double len_oxides;
  double len_stable_phases;

  char major_oxide_names[100][100];
  char stable_phases_names[100][100];

  double major_oxide_propertions[100][100];
  double all_properties[100][100];

} outdataCPP;

// lazy_pointer* runMAGEMin(	int argc, char ** argv, bulk_info z_b, Databases
// DB, global_variable gv, lazy_pointer *lPointer);
...
// void freeMemory_callFromcpp(bulk_info z_b, Databases DB, global_variable gv);
// void clearMemory(bulk_info z_b, Databases DB, global_variable gv);
...
global_variable ReadCommandLineOptions(global_variable gv, bulk_info *z_b,
                                       int argc, char **argv);
...
/* function converting the solver status code to human-readable text and
 * printing it to screen */
void PrintStatus(int status);
#endif

/*---------------------------------------------------------------------------*/
// Defining variables here; Variable defination need to be done only once
