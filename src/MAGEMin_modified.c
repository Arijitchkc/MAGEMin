...
void runMAGEMin(int argc, char **argv, bulk_info z_b, Databases DB, global_variable gv, outdataCPP *dataOut)
{
	/**
	...
	*/
	...
	fill_Cpp_returnDataStructures(gv,												/** global variables (e.g. Gamma) 	*/
											z_b,											/** bulk-rock informations 			*/
											DB.PP_ref_db,									/** pure phase database 			*/
											DB.SS_ref_db,									/** solution phase database 		*/
											DB.cp,
...
void fill_Cpp_returnDataStructures(global_variable gv, bulk_info z_b, PP_ref *PP_ref_db, SS_ref *SS_ref_db, csd_phase_set *cp, stb_system *sp, outdataCPP *dataOut)
{
	/**
	...
	for (int m = 0; m < gv.n_cp_phase; m++) /**  Major oxide Proportions of Other systems */
	{
	...
	for (int i = 0; i < gv.len_cp; i++)
	{
		if (cp[i].ss_flags[1] == 1)
		{
			...
			dataOut->all_properties[counter+1][3]=cp[i].volume*10.*cp[i].ss_n_mol*cp[i].factor;
			...
			dataOut->all_properties[counter+1][11]=sqrt((cp[i].phase_bulkModulus/10. +4.0/3.0*cp[i].phase_shearModulus/10.)/(cp[i].phase_density/1e3));
			dataOut->all_properties[counter+1][12]=sqrt(cp[i].phase_shearModulus/10.0/(cp[i].phase_density/1e3));
			n += 1;
			counter += 1;
		}
	}
	...
	// 	G += z_b.bulk_rock[j]*gv.gam_tot[j];
	// }
	// printf( "%6s %14s %+13.5f %17s %+17.5f %+12.5f %+12.5f %12s %+12.6f %+14.4f %+12.5f %+12.5f %+12.5f %+12.5f\n",
	// 		"SYS",
	// 		" ",
	// 		G,
	// 		" ",
