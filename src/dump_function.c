...
void dump_init(global_variable gv) {
  ...
  if (gv.output_matlab >= 1) {
    ...
    // Deleting MPI_TEMPORARY
    // #ifdef USE_MPI
    // 	if (numprocs==1){	sprintf(out_lm,	"%s_matlab_output.txt"
    // ,gv.outpath); 		} 	else 			{	sprintf(out_lm,
    // "%s_matlab_output.%i.txt"	,gv.outpath, rank); } #else
  ...
  if (gv.verbose == 0) {
    ...
    // Deleting MPI_TEMPORARY
    // #ifdef USE_MPI
    // 	if (numprocs==1){	sprintf(out_lm,	"%s_gui_output.txt"
    // ,gv.outpath); 		} 	else 			{	sprintf(out_lm,
    // "%s_gui_output.%i.txt"	,gv.outpath, rank); } #else 	sprintf(out_lm,
    // "%s_gui_output.txt"		,gv.outpath); #endif

    loc_min = fopen(out_lm, "w");
    fprintf(loc_min, "// {number status[] P[kbar] T[C] G_sys[G] BR_norm[wt] "
                     "Gamma[G] Vp[km/s] Vs[km/s] entropy[J/K]} nextline "
                     "{Phase[name] mode[wt] density[kg.m-3] x-eos}\n");
    fclose(loc_min);
  }
}

void reset_output_struct(global_variable gv, bulk_info z_b, stb_system *sp) {
  ...
  sp[0].shearModulus_S = gv.solid_shearModulus, sp[0].Vp_S = gv.solid_Vp,
...
void mSS_output_struct(global_variable gv, simplex_data *splx_data,
                       bulk_info z_b,

                       PP_ref *PP_ref_db, SS_ref *SS_ref_db, csd_phase_set *cp,
                       stb_system *sp) {
  ...
  /* get LP assemblage - This routine retrieves the information of the solution
   * phases and pure phase as computed at equilibrium -> to be used as initial
   * guess */
  m = 0;
  ...
  for (i = 0; i < d->n_Ox; i++) {
    ...
    if (d->ph_id_A[i][0] ==
        0) { /* if phase is a fake oxide (This may happen for extremely reduced
                composition systems) */
    ...
    if (d->ph_id_A[i][0] == 1) { /* if phase is a pure species */
      for (int j = 0; j < gv.len_ox; j++) {
        sp[0].mSS[m].comp_Ppc[j] =
            PP_ref_db[ph_id].Comp[j] * PP_ref_db[ph_id].factor;
      }
      ...
      sp[0].n_mSS += 1;
      m += 1;
    } else if (d->ph_id_A[i][0] == 2 &&
               strcmp(gv.research_group, "tc") ==
                   0) { /* pure endmembers as solution phase */
      em_id = d->ph_id_A[i][3];
      ...
      (*P2X_read[ph_id])(&SS_ref_db[ph_id], gv.bnd_val);
      ...
      if (strcmp(gv.research_group, "tc") == 0) {
        SS_ref_db[ph_id] =
            PC_function(gv, PC_read, SS_ref_db[ph_id], z_b, ph_id);
      } else if (strcmp(gv.research_group, "sb") == 0) {
        SS_ref_db[ph_id] =
            SB_PC_function(gv, PC_read, SS_ref_db[ph_id], z_b, ph_id);
      }

      for (j = 0; j < gv.len_ox; j++) {
        sp[0].mSS[m].comp_Ppc[j] =
            SS_ref_db[ph_id].ss_comp[j] * SS_ref_db[ph_id].factor;
      }
      ...
      for (j = 0; j < SS_ref_db[ph_id].n_em; j++) {
        ...
        sp[0].mSS[m].mu_Ppc[j] =
            SS_ref_db[ph_id].mu[j] * SS_ref_db[ph_id].z_em[j];
      }
  ...
  if (m >= gv.max_n_mSS) {
    printf("WARNING: maximum number of metastable pseudocompounds has been "
           "reached, increase the value in tc_gss_init_function.c "
           "(SP_INIT_function)\n");
  }
}
...
*/
void fill_output_struct(global_variable gv, simplex_data *splx_data,
                        bulk_info z_b,
  ...
  for (int i = 0; i < gv.len_cp; i++) {
    if (cp[i].ss_flags[1] == 1) {
      ...
      sp[0].SS[m].Vp = sqrt((cp[i].phase_bulkModulus / 10. +
      ...
      for (j = 0; j < cp[i].n_xeos; j++) {
        strcpy(sp[0].SS[m].compVariablesNames[j],
               SS_ref_db[cp[i].id].CV_list[j]);
      }
      ...
      for (j = 0; j < cp[i].n_sf; j++) {
        strcpy(sp[0].SS[m].siteFractionsNames[j],
               SS_ref_db[cp[i].id].SF_list[j]);
      }
      ...
      for (j = 0; j < cp[i].n_em; j++) {
        ...
        for (k = 0; k < gv.len_ox; k++) {
          sum_em_wt +=
              SS_ref_db[cp[i].id].Comp[j][k] * cp[i].p_em[j] * z_b.masspo[k];
        }
        ...
        for (k = 0; k < gv.len_ox; k++) {
          ...
          sp[0].SS[m].emComp_wt[j][k] =
              sp[0].SS[m].emComp[j][k] * z_b.masspo[k];
          sp[0].SS[m].emComp_apfu[j][k] =
              SS_ref_db[cp[i].id].Comp[j][k] * z_b.cpo[j];
          ;
  ...
  for (int i = 0; i < gv.len_pp; i++) {
    ...
    if (gv.pp_flags[i][1] == 1 && gv.pp_flags[i][4] == 0) {
      ...
      sp[0].n_PP += 1;
      sp[0].cp_wt +=
          PP_ref_db[i].phase_cp * gv.pp_n_wt[i] * PP_ref_db[i].factor;
      ...
      sp[0].PP[m].cp =
          (PP_ref_db[i].phase_cp * PP_ref_db[i].factor) / phase_mass;
      ...
      sp[0].PP[m].Vp =
          sqrt((PP_ref_db[i].phase_bulkModulus / 10. +
                4.0 / 3.0 * PP_ref_db[i].phase_shearModulus / 10.) /
               (PP_ref_db[i].phase_density / 1e3));
      sp[0].PP[m].Vs = sqrt(PP_ref_db[i].phase_shearModulus / 10.0 /
                            (PP_ref_db[i].phase_density / 1e3));
  ...
  /* The following section normalizes the entries for S (solid), M (melt) and F
   * (fluid) which are entries useful for geodynamic coupling */
  ...
  mSS_output_struct(gv, splx_data, z_b, PP_ref_db, SS_ref_db, cp, sp);
}

/**
        thermocalc like output
*/
void output_thermocalc(global_variable gv, bulk_info z_b,
  ...
  // Deleting MPI_TEMPORARY
  // #ifdef USE_MPI
  // 	if (numprocs==1){	sprintf(out_lm,	"%s_thermocalc_style_output.txt"
  // ,gv.outpath); 	} 	else 			{	sprintf(out_lm,
  // "%s_thermocalc_style_output.%i.txt"		,gv.outpath, rank); }
  // #else
  // 	sprintf(out_lm,	"%s_thermocalc_style_output.txt"
  // ,gv.outpath); #endif
  ...
  for (i = 0; i < gv.len_cp; i++) {
    if (cp[i].ss_flags[1] == 1) {
      fprintf(loc_min, " %5s", cp[i].name);
      for (j = 0; j < (cp[i].n_sf); j++) {
        fprintf(loc_min, "%10.5f ",
                cp[i].sf[j]); // *-1.0 because inequality are given as -x <= 0
                              // in NLopt
      }
      for (k = j; k < gv.len_ox; k++) {
        fprintf(loc_min, "%10s ", "-");
  ...
  fprintf(
      loc_min,
      "%6s%12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s\n",
      "phase", "mod[mol fr]", "f", "G[J]", "V[cm3/mol]", "Cp[kJ/K]",
      "Rho[kg/m3]", "Alpha[1/K]", "Entropy[J/K]", "Enthalpy[J]", "BulkMod[GPa]",
  ...
  for (i = 0; i < gv.len_cp; i++) {
    if (cp[i].ss_flags[1] == 1) {
      ...
      fprintf(
          loc_min,
          "%+12.5f %+12.5f %+12.5f %+12.5f %+12.5f %+12.5f %+12.8f %+12.6f "
          "%+12.6f %+12.2f %+12.2f %+12.2f %+12.2f",
          cp[i].ss_n, cp[i].factor, G, cp[i].volume * 10., cp[i].phase_cp,
          cp[i].phase_density, cp[i].phase_expansivity, cp[i].phase_entropy,
          cp[i].phase_enthalpy, cp[i].phase_bulkModulus / 10.,
  ...
  for (i = 0; i < gv.len_pp; i++) {
    if (gv.pp_flags[i][1] == 1) {
      ...
      fprintf(loc_min,
              "%+12.5f %+12.5f %+12.5f %+12.5f %+12.5f %+12.5f %+12.8f %+12.6f "
              "%+12.6f %+12.2f %+12.2f %+12.2f %+12.2f",
              gv.pp_n[i], PP_ref_db[i].factor, PP_ref_db[i].gbase,
              PP_ref_db[i].volume * 10., PP_ref_db[i].phase_cp,
              PP_ref_db[i].phase_density, PP_ref_db[i].phase_expansivity,
              PP_ref_db[i].phase_entropy, PP_ref_db[i].phase_enthalpy,
              PP_ref_db[i].phase_bulkModulus / 10.,
              PP_ref_db[i].phase_shearModulus / 10.,
              sqrt((PP_ref_db[i].phase_bulkModulus / 10. +
                    4.0 / 3.0 * PP_ref_db[i].phase_shearModulus / 10.) /
                   (PP_ref_db[i].phase_density / 1e3)),
              sqrt(PP_ref_db[i].phase_shearModulus / 10.0 /
                   (PP_ref_db[i].phase_density / 1e3)));
      fprintf(loc_min, "\n");
    }
  }
  ...
  fprintf(loc_min,
          "%6s %24s %+12.5f %+12.5f %+12.5f %+12.5f %12s %+12.6f %+12.6f "
          "%+12.5f %+12.5f %+12.5f %+12.5f\n",
          "SYS", " ", G, gv.system_volume * 10., gv.system_cp,
          gv.system_density, " ", gv.system_entropy, gv.system_enthalpy,
          gv.system_bulkModulus, gv.system_shearModulus, gv.system_Vp,
          gv.system_Vs);
  ...
  fprintf(loc_min,
          "%% ----------------------------------------------------------\n");
  fprintf(loc_min, "%% at P =  %12.8f, T = %12.8f, for: ", z_b.P,
          z_b.T - 273.15);
  for (i = 0; i < gv.len_cp; i++) {
    if (cp[i].ss_flags[1] == 1) {
...
*/
void output_gui(global_variable gv, bulk_info z_b,
  ...
  // Deleting MPI_TEMPORARY
  // #ifdef USE_MPI
  // 	if (numprocs==1){	sprintf(out_lm,	"%s_pseudosection_output.txt"
  // ,gv.outpath); 		} 	else 			{	sprintf(out_lm,
  // "%s_pseudosection_output.%i.txt"	,gv.outpath, rank); } #else
  ...
  loc_min = fopen(out_lm, "a");
  fprintf(loc_min, "%i %i %.10f %.10f %.10f %.10f", gv.numPoint + 1, gv.status,
          z_b.P, z_b.T - 273.15, gv.G_system, gv.BR_norm);
  ...
  fprintf(loc_min, " %.10f %.10f %.10f", gv.system_Vp, gv.system_Vs,
          gv.system_entropy);
  ...
  for (i = 0; i < gv.len_cp; i++) {
    if (cp[i].ss_flags[1] == 1) {
      ...
      if (n_solvi[cp[i].id] > 1) {
        fprintf(loc_min, "%s_%d \t %.10f \t %.10f \t", cp[i].name,
                n_solvi[cp[i].id], cp[i].ss_n, cp[i].phase_density);
      } else {
        fprintf(loc_min, "%s \t %.10f \t %.10f \t", cp[i].name, cp[i].ss_n,
                cp[i].phase_density);
      }
  ...
  for (i = 0; i < gv.len_pp; i++) {
    if (gv.pp_flags[i][1] == 1) {
      fprintf(loc_min, "%s \t %.10f \t %.10f \t", gv.PP_list[i], gv.pp_n[i],
      ...
      for (int j = 0; j < gv.len_ox; j++) {
        ...
        // fprintf(loc_min, 	"%.10f ",
        // PP_ref_db[i].Comp[j]*PP_ref_db[i].factor);
      }
...
*/
void output_matlab(global_variable gv, bulk_info z_b,
  ...
  for (i = 0; i < gv.len_cp; i++) {
    if (cp[i].ss_flags[1] == 1) {
      ...
      for (j = 0; j < (cp[i].n_sf); j++) {
        fprintf(loc_min, "%8s ",
                SS_ref_db[cp[i].id].SF_list[j]); // *-1.0 because inequality are
                                                 // given as -x <= 0 in NLopt
      }
      for (k = j; k < 18; k++) {
        fprintf(loc_min, "%8s ", "-");
      }
      fprintf(loc_min, "\n");
      for (j = 0; j < (cp[i].n_sf); j++) {
        fprintf(loc_min, "%8.5f ", cp[i].sf[j]); // *-1.0 because inequality are
                                                 // given as -x <= 0 in NLopt
      }
      for (k = j; k < 18; k++) {
        fprintf(loc_min, "%8s ", "-");
  ...
  for (i = 0; i < gv.len_cp; i++) {
    if (cp[i].ss_flags[1] == 1) {
      ...
      fprintf(
          loc_min,
          "%+15.5f %+13.5f %+17.5f %+17.5f %+12.5f %+12.5f %+12.8f %+12.6f "
          "%+14.4f %+12.2f %+12.2f %+12.2f %+12.2f",
          sp[0].ph_frac_wt[n], G, cp[i].volume * 10.,
          cp[i].volume * 10. * cp[i].ss_n_mol * cp[i].factor, cp[i].phase_cp,
          cp[i].phase_density, cp[i].phase_expansivity, cp[i].phase_entropy,
          cp[i].phase_enthalpy, cp[i].phase_bulkModulus / 10.,
          cp[i].phase_shearModulus / 10.,
          sqrt((cp[i].phase_bulkModulus / 10. +
  ...
  for (i = 0; i < gv.len_pp; i++) {
    if (gv.pp_flags[i][1] == 1) {
      ...
      fprintf(loc_min,
              "%+15.5f %+13.5f %+17.5f %+17.5f %+12.5f %+12.5f %+12.8f %+12.6f "
              "%+14.4f %+12.2f %+12.2f %+12.2f %+12.2f",
              sp[0].ph_frac_wt[n], PP_ref_db[i].gbase,
              PP_ref_db[i].volume * 10.,
              PP_ref_db[i].volume * 10. * gv.pp_n[i] * PP_ref_db[i].factor,
              PP_ref_db[i].phase_cp, PP_ref_db[i].phase_density,
              PP_ref_db[i].phase_expansivity, PP_ref_db[i].phase_entropy,
              PP_ref_db[i].phase_enthalpy, PP_ref_db[i].phase_bulkModulus / 10.,
              PP_ref_db[i].phase_shearModulus / 10.,
              sqrt((PP_ref_db[i].phase_bulkModulus / 10. +
                    4.0 / 3.0 * PP_ref_db[i].phase_shearModulus / 10.) /
                   (PP_ref_db[i].phase_density / 1e3)),
  ...
  fprintf(loc_min,
          "%6s %14s %+13.5f %17s %+17.5f %+12.5f %+12.5f %12s %+12.6f %+14.4f "
          "%+12.5f %+12.5f %+12.5f %+12.5f\n",
          "SYS", " ", G, " ", gv.system_volume * 10., gv.system_cp,
          gv.system_density, " ", gv.system_entropy, gv.system_enthalpy,
          gv.system_bulkModulus, gv.system_shearModulus, gv.system_Vp,
...
/**
  Save final result of minimization
*/
void save_results_function(global_variable gv, bulk_info z_b,
...
void mergeParallelFiles(global_variable gv) {
  ...
  fprintf(
      fp2,
      "// "
      "NUMBER\tSTATUS[S,R1,R2,F]\tP[kbar]\tT[C]\tG_sys[G]\tBR_norm[wt]\tVp[km/"
      "s]\tVs[km/s]\tGAMMA[G]; PHASE[name]\tMODE[wt]\tRHO[kg.m-3]\tX-EOS\n");
  ...
  for (i = 0; i < numprocs; i++) {
    ...
    fgets(buf, MAX_LINE_LENGTH,
          fp1); // skip first line = comment (we don't want to copy that)
...
void mergeParallel_LocalMinima_Files(global_variable gv) {
  ...
  fprintf(fp2, "// NUMBER\t INITIAL ENDMEMBER "
               "PROPORTIONS[n+1]\tINITIAL_GUESS_x_eos[n]\tFINAL_x-eos[n]"
               "\tFINAL ENDMEMBER PROPORTIONS[n+1]\tDRIVING_FORCE[dG]\n");
  ...
  for (i = 0; i < numprocs; i++) {
    ...
    fgets(buf, MAX_LINE_LENGTH,
          fp1); // skip 1th line = comment (we don't want to copy that)
    fgets(buf, MAX_LINE_LENGTH,
          fp1); // skip 2nd line = comment (we don't want to copy that)
    if (i > 0) {
      fgets(buf, MAX_LINE_LENGTH,
            fp1); // skip 3rd line = info about Gamma (only needed once)
    }
